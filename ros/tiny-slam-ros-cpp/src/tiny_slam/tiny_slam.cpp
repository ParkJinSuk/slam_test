...
ServiceTinySLAM::ServiceTinySLAM(ros::NodeHandle nh)
{
  nh_ = nh;
 ...
 */
void ServiceTinySLAM::init_tiny_slam()
{
  ...
  TopicWithTransform<sensor_msgs::LaserScan> scan_observer(nh_,
                                                           "laser_scan", frame_odom, ros_tf_buffer_size,
                                                           ros_filter_queue, ros_subscr_queue);
  scan_observer.subscribe(slam);

  std::shared_ptr<RvizGridViewer> viewer(
      new RvizGridViewer(nh_.advertise<nav_msgs::OccupancyGrid>("/map", 5),
